...
for step in tqdm(range(20000)):
    ro[0:1,:] = (2*(np.sum(f[[3,6,7],0,:],axis=0)) + np.sum(f[[1,5,8],0,:],axis=0))/ (1 - (np.sqrt((u_0[0,:,0])**2 + u_0[0,:,1]**2)))
    ...
    for i in range(9):
        f_eq[i,:,:] = weights[i]*ro[:,:] * (1 + np.dot(u_0[:,:],e[i]) + 4.5*np.dot(u_0[:,:],e[i])**2 - 1.5*(u_0[:,:,0]**2 + u_0[:,:,1]**2))
